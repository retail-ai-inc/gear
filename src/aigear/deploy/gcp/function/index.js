...
const vmConfig = {
  ...
  serviceAccounts: [
    {
      email: `200251827214-compute@developer.gserviceaccount.com`,
      scopes: [
...
functions.cloudEvent('cronjobProcessPubSub', cloudEvent => {
  ...
  vmConfig.metadata.items[0].value = commonCommand.replace(/\${dockerImage}/g,dockerImage).replace('createVMDate',JSON.stringify(cronjobInfo));
